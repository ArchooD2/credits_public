def run_credits():
    ...
    This function initializes the animation controller, sets up event sequences,
    and manages playback of the credits audio. It also handles user input for
    skipping to specific scenes and controlling playback speed.

    Side Effects:
        Modifies global variables `beat` and `skip_by`.
        Plays audio using the Playback library.
        Clears the console screen.
    """
    #print("Hello from credits!")  # simple test output
    ...
    def skip_beats(ctr, amount, next_debug):
        """Skips a specified number of beats in the animation controller.

        This function advances the beat counter by a given amount, updates the global
        `beat` value accordingly, and modifies the controller's timeline if debugging is enabled.
        It also updates the global `skip_by` to reflect the new playback offset.

        Args:
            ctr (SceneManager): The animation controller managing scenes and events.
            amount (int): The number of beats to skip forward.
            next_debug (int): The beat index at which to inject a debug scene event
                if debugging is active.

        Side Effects:
            Modifies the global `beat` and `skip_by` variables.
            Optionally injects a debug event into the controller's event sequence.
        """
        global beat
        global skip_by
        ...
        if debug:
            controller.events[next_debug] = (
                am.Event(next_debug, am.Event.layer_scene("debug_counter")),
            )
    ...
    ocean_events = (
        am.Event(
            60,
            lambda c: c.set_generator_data("ocean_b", 1, "text", data_strings["ocean_b_0"]),
        ),
        am.Event(
            310, lambda c: c.set_generator_data("ocean_b", 1, "text", "[##CLEAR|60;6")
        ),
        am.Event(310, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 3)),
        am.Event(
            310,
            lambda c: c.set_generator_data(
        ...
        am.Event(
            646, lambda c: c.set_generator_data("ocean_b", 1, "text", "[##CLEAR|60;8")
        ),
        am.Event(
            652,
            lambda c: c.set_generator_data(
                "ocean_b", 1, "text", data_strings["ocean_b_2"], "offset", 0
            ),
        ),
        am.Event(
            656,
        ...
        am.Event(666, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 6)),
        am.Event(
            850,
            lambda c: c.set_generator_data(
                "ocean_b", 0, "ocean_col", Style.NORMAL + Fore.YELLOW
        ...
        am.Event(
            999, lambda c: c.set_generator_data("ocean_b", 1, "text", "[##CLEAR|60;10")
        ),
        am.Event(
            1000,
            lambda c: c.set_generator_data(
        ...
        am.Event(1000, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 13)),
        am.Event(
            1000,
            lambda c: c.set_generator_data(
                "ocean_b", 1, "text", data_strings["ocean_b_3"], "offset", 0
            ),
        ),
        am.Event(1044, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 102)),
        am.Event(1048, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 230)),
        am.Event(1052, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 500)),
        am.Event(1056, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 760)),
        am.Event(1060, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 1600)),
        am.Event(1064, lambda c: c.set_generator_data("ocean_b", 0, "ocean_glitch", 2500)),
    )
    ...
    ocean2_events = (
        am.Event(
            3896,
            lambda c: c.set_generator_data("ocean_c", 1, "text", data_strings["ocean_c_0"]),
        ),
        am.Event(
            3896,
            lambda c: c.set_generator_data("ocean_c", 2, "text", data_strings["ocean_c_1"]),
        ),
        am.Event(
            3896,
            lambda c: c.set_generator_data("ocean_c", 3, "text", data_strings["ocean_c_2"]),
        ),
        am.Event(
            3896,
            lambda c: c.set_generator_data("ocean_c", 4, "text", data_strings["ocean_c_3"]),
        ),
        am.Event(
            4400, lambda c: c.set_generator_data("ocean_c", 1, "text", "[##CLEAR|60;10")
        ),
        *(
            am.Event(
                4401 + i * 2,
                lambda c: c.set_generator_data(
                    ...
                    0,
                    "ocean_col",
                    random.choice((Style.BRIGHT, Style.NORMAL)) + Fore.BLACK,
                ),
            )
            for i in range(5)
        ),
    ...
    controller = am.SceneManager(
        ...
        (
            ...
            *ocean_events,
            ...
            am.Event(
                2348, lambda c: c.set_generator_data("funding", 0, "text", (("",), ("",)))
            ),
            ...
            am.Event(
                3390,
                lambda c: c.set_generator_data(
                    "fundingx2", 0, "text", data_strings["fundingx2_0"]
                ),
            ),
            am.Event(
                3390,
                lambda c: c.set_generator_data(
                    "fundingx2", 1, "text", data_strings["fundingx2_1"]
                ),
            ),
            am.Event(
                3390,
                lambda c: c.set_generator_data(
                    "fundingx2", 3, "text", data_strings["fundingx2_2"]
                ),
            ),
            ...
            *ocean2_events,
            ...
            am.Event(
                5916,
                lambda c: c.set_generator_data(
                    "fdg_down", 1, "text", data_strings["fdg_down_2"]
                ),
            ),
